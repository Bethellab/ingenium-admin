    ...
    <div className={className}>
      <RenderIf condition={!!label}>
        <div className="space-y-1 pb-2">
          <div className="flex justify-between">
            <label
              htmlFor={id}
              className={`font-medium text-label ${labelClassName}`}
            >
              {label}
            </label>
      ...
      <div className="relative w-full input-container">
        ...
        <input
          ...
          className={`  ${variant}
                    p-[12px] h-[40px] text-sm text-primary_100 w-full outline-0 border-stroke hide_tap border-[1px] rounded-lg
                     focus:border-primary focus:border-2 placeholder:text-[#B2B2B2] placeholder:text-sm ${
                       error ? "border-error" : ""
                     }
        ...
        <RenderIf condition={type === "password"}>
          ...
          >
            {!passwordView ? (
              <Icon icon="ph:eye" className="w-4 h-4 text-[#B2B2B2]" />
            ) : (
              <Icon icon="ph:eye-slash" className="w-4 h-4 text-[#B2B2B2]" />
            )}
          </button>
        </RenderIf>
      ...
      <RenderIf condition={!!error}>
        <span className="w-full text-right text-xs text-error">{error}</span>
      </RenderIf>
    </div>
  );
};
